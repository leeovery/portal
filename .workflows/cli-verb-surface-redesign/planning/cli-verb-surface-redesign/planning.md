...
### Phase 14: Analysis (Cycle 8)

Address findings from Analysis (Cycle 8).

#### Tasks

| Internal ID | Name | Edge Cases |
|-------------|------|------------|
| cli-verb-surface-redesign-14-1 | Introduce a typed Domain across the cmd↔resolver routing boundary | Collapse the two runtime domain vocabularies (`cmd.Target.Domain` bare/session/path/zoxide/alias and the resolver `PathResult`/`SessionResult` `Domain` session/path/alias/zoxide/glob/miss) into one typed `Domain` constant set in `internal/resolver`; convert `globExpandableDomain` (open_burst.go) + `resolveOpenSurfaces` (open_surfaces.go) switches to typed constants; the `resolve` log-line `domain` attr must stay byte-identical (produce it from the enum `String()` — session/path/alias/zoxide/miss); cobra flag-name keys in `openTargetPins` legitimately stay string-typed; glob routing + every domain-pin route behave identically; update `open_targets_guard_test`/`open_pin_source_guard_test` to typed values while keeping the flag-set-drift guard; `go build`/`go test ./...`/`golangci-lint run` clean |
| cli-verb-surface-redesign-14-2 | Extract a shared exact-session-match helper and unify its error handling in the resolver | One unexported helper on `QueryResolver` owns `ListSessionNames` + `slices.Contains` + a single lister-error policy (error → no match, matching today's `Resolve` at query.go:138); `Resolve`, `ResolveSessionPin`, `ResolveSessionPinAll` all route through it (the two pin variants currently swallow the lister error via `names, _`); keep `SessionResult.Domain` assigned per call site (session vs glob) — only match/error logic is centralized, not result construction; broader `*All`-vs-single-pin surface symmetry restructure is OUT of scope; happy path identical, lister-error path now consistent across all three (no match, no panic); `go build`/`go test ./...`/`golangci-lint run` clean |
| cli-verb-surface-redesign-14-3 | Route doctor's host-terminal Detector/Resolve through the shared spawn-seam bundle | severity low; remove the hand-built third construction site in `resolveDoctorDeps` (doctor.go:131-158) + its "must be kept in sync by hand" comment; option A route `Detector`/`Resolve` through `buildProductionSpawnSeams(client)` accepting one eager `terminals.json` read on this file-heavy bootstrap-exempt command, OR option B make the bundle's `Resolve` field lazy (defer `buildResolver()`) to preserve the deferred read across all three consumers — confirm open-burst + picker callers unchanged; doctor host-terminal report line behaviourally unchanged (correct identity/resolution, informational-only, no exit-code impact); existing doctor host-terminal tests pass; `go build`/`go test ./...`/`golangci-lint run` clean |
...
### Phase 16: Analysis (Cycle 10)

Address findings from Analysis (Cycle 10).

#### Tasks

| Internal ID | Name | Edge Cases |
|-------------|------|------------|
| cli-verb-surface-redesign-16-1 | Extract shared down-server result helper in doctor.go | severity low; source duplication; Rule-of-Three consolidation — `checkDaemonAlive` (483-487), `checkSaverUp` (511-515), `checkHooksRegistered` (542-546) each open with the identical `!serverUp` guard returning `checkResult{name: name, status: checkFail, detail: doctorRuntimeNotRunning}`; add a `runtimeDownResult(name string) checkResult` helper in `cmd/doctor.go` returning that exact shape and call it from all three `!serverUp` arms; preserve each check's `const name` verbatim so the produced result (name/status/detail) is byte-identical; no behaviour change — doctor per-line output + exit-code contract (0 iff all pass; down server → fail via daemon/saver/hooks) unchanged; existing down-server `checkFail`/`doctorRuntimeNotRunning` tests for daemon/saver/hooks pass unchanged, add or extend a focused unit test asserting `runtimeDownResult(name)` returns `checkFail` + `doctorRuntimeNotRunning` for each of the three check names |
| cli-verb-surface-redesign-16-2 | Remove the fabricated-Domain latent trap in the degenerate single-surface burst | severity low; source architecture; the degenerate single-surviving-surface case round-trips `spawn.Surface`→`resolver.QueryResult` via `surfaceToResult` (open_burst.go:148, :154-163) which *fabricates* `Domain` — every mint→`PathResult{Domain: DomainPath}` regardless of alias/zoxide origin, every attach→`SessionResult{Domain: DomainSession}` (never `DomainGlob`) — dead data today only because `openResolved` reads the result's concrete TYPE (attach vs mint) never `.Domain`, and the resolve line already fired upstream in `resolveOpenSurfaces`; latent trap: any future `openResolved` change consulting `r.Domain` would silently act on `DomainPath` for an alias/zoxide mint; preferred fix — thread the original `resolver.QueryResult` through `resolveOpenSurfaces`/`dispatchOpenBurst` and pass it to `openResolved` (no fabrication), else carry `resolver.Domain` on `spawn.Surface` and reconstruct accurate provenance, else add a focused comment/assert at `surfaceToResult` documenting the fabricated `Domain` is intentionally never consumed; no observable behaviour change — attach-vs-mint dispatch, command guard, ack write, connector dispatch, and the upstream resolve-decision log line all identical; existing single-target attach/mint (incl. alias/zoxide-origin mint) burst tests pass, if threaded add a test asserting an alias/zoxide-origin single mint carries its true domain (not `DomainPath`) at the `openResolved` boundary |

---

### Phase 17: Analysis (Cycle 11)

Address findings from Analysis (Cycle 11).

#### Tasks

| Internal ID | Name | Edge Cases |
|-------------|------|------------|
| cli-verb-surface-redesign-17-1 | Name the host-terminal resolution seam type in internal/spawn | severity low; source architecture; pure naming extraction — declare `type AdapterResolver func(Identity) (Adapter, Resolution)` once in `internal/spawn` (in command.go next to the existing `ExecutableResolver` precedent, with a doc comment describing it as the host-terminal identity→adapter resolution seam shared by the picker, the multi-target open burst, and doctor) and replace the inline `func(spawn.Identity) (spawn.Adapter, spawn.Resolution)` spelling at all 8 sites: 6 struct-field sites (cmd/open.go:662 `openDeps.resolve`, cmd/doctor.go:129 `doctorDeps.Resolve`, cmd/spawn_seams.go:36 `productionSpawnSeams.Resolve`, cmd/open_burst_run.go:33, internal/tui/build.go:54, internal/tui/model.go:465) + 2 function/option-parameter sites (cmd/doctor.go:404 `checkHostTerminal`'s `resolve` param, internal/tui/spawn_detect.go:42 `WithResolve`'s `fn` param); reference it unqualified (`AdapterResolver`) within `internal/spawn` and as `spawn.AdapterResolver` in cmd/ and internal/tui/; do NOT alter the signature, wiring, or behaviour, and leave the separately-named `ExecutableResolver` seam untouched; the seam signature stays byte-identical; no new behavioural tests (pure refactor) — existing spawn/open-burst/doctor/tui suites pass unchanged, and a grep confirms the inline signature no longer appears at any call site with the named type as the single definition point; `go build ./...`/`go test ./...`/`golangci-lint run` clean |

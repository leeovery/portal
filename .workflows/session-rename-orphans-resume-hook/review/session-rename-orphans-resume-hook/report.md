...
5. Consolidate shared test setup into named helpers (each requires a judgement call on coupling vs. duplication):
   - `internal/tmux/hookkey_cross_site_realtmux_test.go:218` and `internal/tmux/hookkey_format_realtmux_test.go:126` — both seed a near-identical 3-pane (split + new-window) stamped session; a shared "seed a 3-pane stamped session and return its pane ids" fixture would remove the duplication (Report 2-5)
   - `internal/restore/rename_reboot_hook_integration_test.go:344` / `rename_reboot_durability_integration_test.go:307` — `assertHookFireCount`, `findCapturedSession`, `verifyHookKeyed`, and shared consts live in whichever file the analysis cycles settled them in; moving them to one clearly-named file (e.g. `rename_reboot_shared_test.go`) would make ownership explicit rather than implicit in a sibling test's body (Report 3-5)
6. `internal/restore/rename_reboot_durability_integration_test.go:148-159,164-167` — the durability leg calls `state.CaptureStructure` twice back-to-back; the subtest's captured index could be threaded through to cycle 2 instead of re-capturing. Defensible as-is (self-contained subtest, cheap re-capture); decide whether to dedupe (Report 3-6)
7. `internal/restore/multipane_legacy_integration_test.go:141-146` — the base-index-drift edge case (assert against saved-index keys under drift) is documented but not exercised by any fixture; decide whether a drift-inducing variant is worth adding for defence-in-depth (not required by the acceptance criteria) (Report 3-7)

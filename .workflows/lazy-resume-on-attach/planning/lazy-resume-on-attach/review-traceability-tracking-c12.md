...
**Resolution**: Fixed — all eight sites applied: Phase 5's Planner's-call bullet and the `lazy-resume-on-attach-5-3` table-row clause in `planning.md`, and task 5-3's Problem, Do, Acceptance Criteria, Tests, Edge Cases and Context. The waiter now resolves an `\x1b` against a follow window on `cfg.Settle` before dispatching it, and consumes a sequence to its terminator under a byte cap. Tick body re-synced and byte-verified. Task 4-2 unedited, as the finding states.
**Notes**: Verified independently before applying. The specification does state it outright (line 239): "While the confirmation is up, `y` and Escape are the only keys that act… the pane never acts on a key the screen in front of the user does not offer" — so an arrow cancelling is a spec violation rather than a fork, which is what makes the move settled. Cycle 9 examined the cancel half and cleared it as "the harmless direction"; it did not examine the CSI-final-byte half, which is the new ground here — task 4-2's read loop is stateless one byte at a time, so its Do text's claim that a sequence's bytes are discarded has no mechanism behind it and `\x1b[5d` reaches the `d` handler. Also checked the tension the fix rests on: task 4-2's "the wait path arms no timer" criterion is stated with a colon and a behavioural clause (a held report does not expire), which the follow window satisfies, and task 4-6 already arms a timer on the same loop — so the plan's stated reason for immediate dispatch did not hold.

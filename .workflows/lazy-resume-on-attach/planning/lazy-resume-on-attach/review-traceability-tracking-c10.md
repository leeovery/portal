...
**Resolution**: Fixed — Phase 6's reference-frame bullet and task 6-8's Context each name the divergence and state that the specification's form governs while the frame is read for layout, structure and colour-role match. Nothing about what is built changed. Tick body re-synced and byte-verified.
**Notes**: Verified independently before applying. Opened the committed frame: `folio-Jiz4el` is the only two-indicator row and its green dot sits roughly two cell-pitches left of the orange one, where every single-indicator row draws in the same rightmost cell — one blank cell between the pair, as the finding states. Checked the derivation rather than taking it: §8.3 (specification line 375) states `AP` when both with "no second row geometry", so the letters sit in the cells their dots occupy, and the plan's own criterion that the row's total width is identical across all four combinations (task 6-5) would break if the coloured pair were three cells wide against a colourless two. Both legs hold, so the move stands as settled rather than a fork for the user.

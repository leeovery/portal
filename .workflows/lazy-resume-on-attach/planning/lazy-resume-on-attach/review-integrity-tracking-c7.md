...
**Resolution**: Fixed — task 4-5's `resolveResumeDecision` clause now says the lookup breadcrumbs are factored into one unexported helper both sites call, naming why (`execShellOrHookAndExit` goes on emitting them verbatim on its nil-`Decision` path, where the existing direct callers pin them). Tick body re-synced and byte-verified.
**Notes**: Verified in the tree before applying: the four `hook lookup` DEBUGs and the `lookup on-resume hook failed` WARN are inside `execShellOrHookAndExit` at `cmd/state_hydrate.go:175-191`, and the two direct-caller suites are `cmd/state_hydrate_exec_log_test.go` and `cmd/hooks_read_lock_test.go`. The acceptance criterion and edge case needed no change — they already stated the outcome the Do contradicted.

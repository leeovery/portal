...
The settled panel is painted into the pane's **alternate screen**, and the saver reads a pane exactly the way that returns it: `capture-pane -e -p -S -` with no `-a` (`internal/tmux/tmux.go:752`), which the prompt-surface measurement shows returns the card rather than the transcript underneath. So an unfrozen waiting pane would hash differently from its last write within a tick, and the saver would replace that pane's saved transcript with a picture of the resume card — the exact corruption this section exists to prevent, and the one that compounds, because the next reboot replays the picture and draws a live card over it.

The decision stands unchanged. What changed is that it is the only thing standing between a waiting pane and the loss of the transcript the user is waiting to come back to, which raises how durably the freeze is held.

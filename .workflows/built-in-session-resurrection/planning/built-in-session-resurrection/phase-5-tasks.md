...
- Create `cmd/bootstrap/bootstrap.go`:
  ...
  - `func (o *Orchestrator) Run(ctx context.Context) (bool, error)`:
    ...
    6. `if err := o.Restoring.Clear(); err != nil` → fatal — wrap as `*FatalError("Portal failed to clear @portal-restoring marker: " + err.Error(), err)`, log ERROR via `ComponentBootstrap`, and return. The marker MUST NOT leak past bootstrap: a stuck `@portal-restoring` would suppress every subsequent daemon tick across this server's lifetime, silently breaking persistence. Per spec § Fatal Bootstrap Errors and `CLAUDE.md` bootstrap step 6 ("Clear `@portal-restoring` — fatal on failure (the marker must not leak past bootstrap)"). _Cycle-1 review reconciliation: an earlier draft of this body characterised the failure as soft + WARN; spec, CLAUDE.md, and the `cmd/bootstrap/bootstrap.go` step-6 implementation all classify it as fatal — wording corrected here._
...
- Create `cmd/bootstrap/bootstrap_test.go`:
  ...
  - `"it returns a FatalError when Restoring.Clear fails"`: recorder errors on `Restoring.Clear`; `Run` returns `*FatalError` whose `UserMessage` begins `"Portal failed to clear @portal-restoring marker: "` and whose `Cause` unwraps to the recorder's sentinel; an ERROR log entry on `ComponentBootstrap` precedes the return.
...
- [ ] `Restoring.Clear` failure logs ERROR (via `ComponentBootstrap`) and returns `*FatalError` — fatal path per spec § Fatal Bootstrap Errors and `CLAUDE.md` bootstrap step 6. The marker must not leak past bootstrap.
...
- `"it returns a FatalError when Restoring.Clear fails"`
...
- `Restoring.Clear` failure is fatal (log ERROR + return `*FatalError`). The marker MUST NOT leak past bootstrap — a stuck `@portal-restoring` suppresses every subsequent daemon tick for the lifetime of this tmux server. Per spec § Fatal Bootstrap Errors and `CLAUDE.md` bootstrap step 6 ("Clear `@portal-restoring` — fatal on failure"). _Cycle-1 reconciliation note: prior wording in this section called it soft; spec / CLAUDE.md / implementation are the authority and they classify it fatal._

...
- Fish is not installed on the development machine, so its leg is verified by the skipping subtest plus a manual `fish -c` run wherever fish is available; the emitted-text assertions run unconditionally in both lanes

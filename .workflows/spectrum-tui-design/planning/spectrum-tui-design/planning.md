...
| Internal ID | Name | Edge Cases |
|-------------|------|------------|
| spectrum-tui-design-10-1 | Extract the shared pad-right geometry behind a fill-style parameter — `headerPadRight` (header.go:224-230) and `noticeBandPadRight` (notice_band.go:329-335) are byte-for-byte structurally identical, differing only by which fill style paints the trailing pad; extract `padRightWithStyle(seg string, segWidth, w int, fill lipgloss.Style) string` and keep both as thin delegating wrappers | single `padRightWithStyle` owns the return-if-full-else-join-styled-pad geometry, `headerPadRight`/`noticeBandPadRight` thin wrappers supplying only their fill style (signatures unchanged), no call site modified, unrelated `padTo`/`padLineToCanvasWidth` untouched (distinct shapes), imports stay satisfied, byte-identical header + notice-band render for all width/mode/colourless/tint combos, consolidation-style guard table test pins the shared geometry |

...
- **Dependency wiring (pinned).** `runHookStaleCleanup(lister AllPaneLister, store *hooks.Store, logger *slog.Logger, swallowListError bool, onRemoved func(string))` takes five arguments; the daemon supplies them as:
  ...
  - **`swallowListError`** = **`true`** (matches "log WARN and retry next cadence; never escalate/crash" — the `portal clean` posture, not bootstrap's `false`).
  - **`onRemoved`** = **`nil`** (a background daemon emits no per-removal foreground output; the `EmitCleanStaleSummary` breadcrumb is the audit record).

...
- The test-local `cleanStaleAdapterT` re-implements the six-branch algorithm verbatim from Task 2-2's Do step 3. This is the spec-aligned shape (production not refactored). Drift risk is mitigated by the algorithm being short and fully specified, plus Phase 3 integration tests exercising the real production adapter end-to-end.

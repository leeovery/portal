...
- In `cmd/spawn.go` `runSpawn` — the not-all-confirmed branch after `burster.Run` + `Ack.Clean(batch)`:
  ...
  - `return fmt.Errorf("spawn: failed to open window(s) for %s — others left open", quoteJoin(failed))` — a plain, non-`UsageError`, non-silenced error → exit 1 on stderr. The opaque `Result.Detail` for each failure goes only to the DEBUG log, never the user-facing message.

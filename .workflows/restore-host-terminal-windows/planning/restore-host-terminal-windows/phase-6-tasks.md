...
- In `internal/tui/model.go`, complete the `case spawnCompleteMsg:` arm (the branch not taken by Task 6.4's `allConfirmed`):
  ...
  - Set the transient flash:
    ...
    - Else: `m.setFlash(fmt.Sprintf("⚠ %s failed to open — others left open", spawn.QuoteJoin(failedNames)))` naming every failed window (mirrors the design/CLI copy; opaque `Result.Detail` goes only to the DEBUG log — 6-10 — never the user message).

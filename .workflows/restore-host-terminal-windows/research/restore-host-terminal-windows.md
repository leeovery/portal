...
**OPEN — is the bundle id the *right* identity key? (user devil's-advocate, deep-dive pending):** bundle id is verified-working and is the canonical macOS app identity (Launch Services / TCC / `open -b` all use it — *not* a smell). Two unresolved concerns: (1) **separation** — the canonical *internal* matching key need not be the *user-facing* config key; a friendly alias (`ghostty`) mapping to bundle id(s) may be the better UX, with Portal *showing* the detected id. (2) **stability/future-proofing** — bundle ids change across channels/forks/rebrands (Warp = `dev.warp.Warp-Stable` implies `-Preview` siblings → adapters likely match a *family* `dev.warp.*`, not a literal). Deeper truth: *any* key leaves us hostage to the terminal author (bundle id, AppleScript dict, spawn command all churn), so "future-proof" = most-stable-available + graceful degradation + easy-to-patch, not immune. **Not yet researched against alternatives / other tools' practice — focused deep-dive warranted before discussion.**

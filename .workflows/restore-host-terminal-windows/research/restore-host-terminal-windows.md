...
- **Unknown-terminal keying — NOT a chicken-and-egg (user follow-up).** The Portal-managed friendly alias only covers *known* terminals; an unknown terminal has no pre-baked alias. But the user never *guesses* the identity — the walk recovers both the **bundle id** *and* the **`.app` name** (`Warp`), and Portal **displays** them (unsupported banner / detect command), so the config key is copy-paste. Crucially, the custom-terminal config can key off the friendly **`.app` name** (`Warp`), not the raw bundle id — exactly what **VS Code** does (`terminal.external.osxExec` = `iTerm.app`/`WezTerm.app`). Tradeoff: `.app` name is slightly less collision-resistant, but fine for a user's own machine. **The irreducible floor:** for an unknown terminal, the *only* genuinely user-authored input is the **spawn command** (how *that* terminal opens a window) — Portal can't know that for an app it's never seen. Identity is always detected-and-shown; only the spawn recipe is authored. Not painful: "here's your terminal's name — now tell me how to open a window in it."

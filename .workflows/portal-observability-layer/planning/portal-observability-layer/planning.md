...
| Internal ID | Name | Edge Cases |
|-------------|------|------------|
| portal-observability-layer-10-1 | Extend internal/logtest.Sink to expose attr values and collapse the 11 structured-attr-map capture handlers onto it | add `Attrs map[string]slog.Value` to `logtest.Record` populated in `Sink.Handle`'s existing bound+call loop (no second traversal); keep `Keys` unchanged so c3 keys-asserting consumers still pass; last-write-wins on duplicate keys; add value-typed accessors (`AttrString`/`IntAttr`/`DurationAttr`/`OnlyRecord`/message-filter) matching the store copies' `*testing.T`-first signatures; route all eleven test files (alias/hooks/project/storelog store_logging+store tests, storelog clean_stale, bootstrap clean_sweep_summary, cmd state_daemon_cycle_summary/config_migrate_logging/state_hydrate_timeout_log/state_hydrate_replayed_log, state fifo_sweep_summary, tmux portal_saver_lifecycle_events) onto `logtest.Sink`, deleting the ~70-line core (struct/record/owner/Enabled/WithAttrs/WithGroup/Handle/all) and keeping only divergent accessors as thin wrappers; four store packages drop their `installCapture`/`onlyRecord`/`attrString` copies + copy-acknowledgment comments; no production (non-`_test.go`) file modified; `internal/logtest` imported only from test files; `go build ./...` + `go test ./...` green with identical assertions |
...
| Internal ID | Name | Edge Cases |
|-------------|------|------------|
| portal-observability-layer-11-1 | Migrate tui previewCaptureSink onto the shared logtest.Sink helper | delete `previewCaptureSink` struct + `owner()`/`Enabled`/`WithAttrs`/`WithGroup`/`Handle`/`body()` (byte-identical copy of `logtest.Sink`); `newTestLogger` routes through `logtest.NewCaptureLogger(t)` bound to `component=preview` returning `*logtest.Sink`; `readLog` reads via `logtest.Sink.Body()`; retype every sink call site from `*previewCaptureSink` to `*logtest.Sink`; drop now-unused imports (context/fmt/strings/sync/handler-slog) only if no longer needed; fixes the inert `WithGroup` receiver-return divergence; no production file modified; rendered-body shape (`<LEVEL> <msg> key=value...`, bound attrs first) unchanged; `go test ./internal/tui/...` + `go build ./...` + `go vet` green |
| portal-observability-layer-11-2 | Align restoretest.OpenTestLogger with the production sink's portal.log file contract | `OpenTestLogger` stops writing a bare regular-file `portal.log` that production's `migrationGuard` (`internal/log/sink.go:304-344`) unlinks on first write when a real binary shares the `stateDir`; preferred: route through `internal/log` handler (verify no import cycle — leaf pkg) producing `portal.log.<date>` + symlink; fallback: write the `.<date>` + symlink shape directly and document the no-shared-stateDir constraint; preserve `*testing.T`-first signature + `*slog.Logger` return + `t.Cleanup` close so `exit_closes_pane_integration_test.go:375` and other call sites compile unchanged; doc comment states the contract; no production runtime behavior changed; `go test ./internal/restoretest/... ./internal/restore/...` + `go build ./...` + `go vet` green, no new cycle |

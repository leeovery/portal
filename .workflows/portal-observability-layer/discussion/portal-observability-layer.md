...
1. Parse `cutoff := today.AddDate(0, 0, -PORTAL_LOG_RETENTION_DAYS)`. Default `PORTAL_LOG_RETENTION_DAYS = 30`. Invalid env value (non-integer, negative, > 365) → use default and emit one WARN: `log-rotate: invalid PORTAL_LOG_RETENTION_DAYS=<v>, using 30d`. (This WARN is distinct from the level-discipline invalid-env handling — retention parsing happens later in the handler's lifecycle, well after `log.Init`'s final-two-actions invariant has been satisfied.)
...
Default `PORTAL_LOG_LEVEL = info`. Invalid env value (any value that is not exactly `debug` / `info` / `warn` / `error` after lowercase) → fall back to `info`. The invalid-env condition is conveyed by the `process: log-level resolved` line itself (via `source=fallback raw=<invalid>`); **no separate WARN line is emitted** (closes review-006 K6: a separate WARN would either precede `process: start` — confusing on grep — or break the locked "final two actions" invariant by adding a third line). The `source=fallback` value is the audit signal; operators grep on `source=fallback` to find invalid-env occurrences.
...
**Closed attr-key value space** (11 contextual + 3 state-mutation + 11 cycle-summary + 7 lifecycle + 3 hydrate + 4 baseline = 39 group slots; 31 distinct keys after the 8 cross-group overlaps enumerated in Current State):
...
`os.Exit` outside `internal/log.Fatal` and `internal/log.Panic` is PROHIBITED in production code: `cmd/`, `main.go`, and any non-`_test.go` file in `internal/`. The prohibition does NOT apply to `_test.go` files (test code may use `os.Exit` per Go convention), `TestMain` functions, or `internal/portaltest` helpers (subprocess lifecycle helpers legitimately call `os.Exit` from spawned test binaries via `cmd.Run`). PR review rejects new direct `os.Exit` call sites in production code only. (Closes review-006 K3.)

**Init re-entry (closes review-004 I6, review-005 J8, and review-006 K2):** `log.Init` is idempotent and re-entrant. The first call constructs the root, wires the handler via an internal `atomic.Pointer[slog.Handler]` indirection, AND emits both lifecycle markers (`process: start` + `process: log-level resolved`). Subsequent calls atomically replace the handler but **DO NOT re-emit** the lifecycle markers — this preserves the paired-start/exit invariant (every `process: start` matches exactly one `process: exit` or `process: exec` per process lifetime). Test harnesses doing capture-redirect mid-test simply call `Init` again with the desired handler-producing args; no separate `SetHandlerForTest` seam is needed (K2: a second seam doing the same thing as Init re-entry would invite drift between production behavior and test-only paths). The internal atomic-handler indirection IS permanent (closes review-006 K7): every `Handle` call dereferences the atomic pointer, so the per-call cost is one atomic load (~nanoseconds, well below slog's per-call cost). The indirection is what makes the cached `var logger = log.For(...)` pattern in consumer packages work after a re-Init.

`log.For` is safe before `Init` — returns a child of a default placeholder root that writes to stderr until `Init` lands; the cached package-level `var logger = log.For(...)` in consumer packages picks up the real root automatically via the atomic handler indirection.
...
**Catalog convention (closes review-006 K5):** Required attrs columns enumerate only **call-site-supplied** attrs. Baseline attrs (`component`, `pid`, `version`, `process_role`) are auto-injected at root logger construction and ride on every line; they're NOT repeated in the catalog. Spec writers reading a row do not pass baseline attrs at the call site.

**Saver lifecycle events (component: `saver`):**

| Site | msg | Required call-site attrs |
|---|---|---|
| Bootstrap creates the `_portal-saver` placeholder pane | `placeholder created` | `tmux_pane` (the pane id) |
...
| Bootstrap observes the daemon process is up and ready (after the 2s readiness barrier) | `daemon ready` | `target_pid` (the daemon pid) |
...
| Site | msg | Required call-site attrs |
|---|---|---|
| Daemon process startup (after `Init` but before tick loop) | `spawn` | `tmux_pane` |
| Daemon acquires `daemon.lock` (post-pre-check) | `lock acquired` | (none — baselines suffice) |
...
- Apply the locked Diagnostic context preservation rule to the hydrate helper's boundary calls — closes review-005 J10. **Hydrate code path scope (closes review-006 K4):** the boundary preservation in PR 1 is pinned to the call sites *in files under `cmd/state_hydrate*.go` only*. Shared helpers called from those sites (`internal/state.TailScrollback`, `*tmux.Client` methods, `internal/state.OpenFIFOForSignal`, etc.) stay unchanged in PR 1 — their boundary wrapping is part of PR 2's codebase-wide sweep. Where a hydrate call site invokes a shared helper that doesn't yet preserve context, the hydrate code wraps the returned error at its own layer (`fmt.Errorf("hydrate: %w", err)`) so the hydrate forensic trail still surfaces a hydrate-attributed error with whatever context the helper already provides. Once PR 2's sweep tightens the helpers, the hydrate-layer wraps remain valid (they don't conflict with deeper context being added below).
...
- **Review-005 (J1–J12)** — all 12 findings addressed via in-place amendments. J1 removed stale "Hook command privacy" from Open Threads (the verbatim posture was locked in State-mutation + Defensive invariants). J2 corrected the invalid-PORTAL_LOG_LEVEL WARN prefix from `bootstrap:` to `process:` (later refined per K6 — see below). J3 added `value`, `via` to the closed attr vocab as a new state-mutation subgroup. J4 added `alias` to the contextual attr vocab. J5 renamed AtomicWrite's failure-phase attr from `error_class` to `failure_phase` to disambiguate the overlapping value space (AtomicWrite failures now carry both `failure_phase=<phase>` AND `error_class=unexpected`). J6 corrected the PR 2 lifecycle catalog count from "12 sites" to "11 INFO + 1 DEBUG-by-rule". J7 superseded the Defensive invariants "final action" wording with "final two actions" to accommodate the log-level resolved line. J8 specified Init re-entry suppresses lifecycle markers to preserve the paired-start/exit invariant. J9 collapsed the original `hydrate: exec` INFO into `process: exec` from `log.Exec` (one terminal-point INFO per exec hand-off, in the correct component). J10 added the diagnostic context preservation sweep for the hydrate code path to PR 1 scope (codebase-wide remains PR 2). J11 enumerated the cross-group attr overlaps explicitly in Current State. J12 answered: the `os.Exit → log.Fatal` migration sweep ships in PR 1 alongside the rest of Defensive invariants.
- **Review-006 (K1–K7)** — all 7 findings addressed via in-place amendments. K1 expanded the closed attr-vocab header from `(10 contextual + 11 cycle-summary + 4 baseline)` to enumerate all 5 groups (11+3+11+7+3+4 = 39 group slots, 31 distinct keys post-overlap). K2 removed the redundant `SetHandlerForTest` seam — tests use `log.Init` re-entry directly, since Init re-entry already provides handler-only swap without lifecycle marker re-emission. K3 carved out the `os.Exit` prohibition to apply to production code only (`cmd/`, `main.go`, non-`_test.go` in `internal/`); `TestMain`, `_test.go` files, and `internal/portaltest` helpers may use `os.Exit` per Go convention. K4 pinned the hydrate boundary preservation scope to `cmd/state_hydrate*.go` only — shared helpers (`TailScrollback`, `*tmux.Client`, etc.) stay in PR 2's codebase-wide sweep; hydrate wraps returned errors at its own layer for forensic continuity. K5 normalized the lifecycle catalog: rows enumerate only call-site-supplied attrs, baselines (`pid`/`version`/`process_role`/`component`) are auto-injected and never repeated. K6 dropped the redundant invalid-PORTAL_LOG_LEVEL WARN — the `process: log-level resolved` line itself carries `source=fallback raw=<invalid>` as the audit signal, preserving the locked final-two-actions invariant. K7 documented the atomic-handler-pointer indirection as a permanent design choice (every Handle call pays one atomic load; this is what makes the cached `var logger = log.For(...)` pattern survive Init re-entry).

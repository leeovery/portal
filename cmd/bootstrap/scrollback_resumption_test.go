...
// if step 9 were disabled — closing the regression-guard contract from
...
// if a future change disables step 9, the primary positive above would
...
func TestScrollbackResumption_WithoutCleanupScrollbackNotSaved(t *testing.T) {
	...
	// in StaleMarkers (the default). Step 9 is effectively disabled. The
	...
	// is a no-op, so step 9 leaves the seeded marker untouched.
	_, found, err := client.TryGetServerOption(markerName)
	if err != nil {
		t.Fatalf("TryGetServerOption %s: %v", markerName, err)
	}

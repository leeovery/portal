...
**Tab completion.** Tab after the session-opening function asks Portal for `open`'s completions: `x <TAB>` offers your live session names, and `x /po<TAB>` completes the term after the slash against the sessions a search can reach — every live session but the one you are attached to, since a search never lands you back where you already are — leaving the slash in place. **A shell has that behaviour only once it has evaluated the current output of `portal init`** — start a new shell, or re-run the `eval` in your profile; the `/term` form itself is `portal open`'s own parsing, independent of the shell integration. The deliberate trade beside it: a path argument after the function does not fall through to filename completion, which is the contract `portal open` has always had. (Switching that fallback off needs a shell that supports it — bash 4+ with `bash-completion`, zsh, or fish. macOS's stock `/bin/bash` 3.2 cannot, so filenames still complete there.)
...
xctl hook list                                   # list hooks: key, event, command, the pane's current location (4th column, empty if no live pane carries the token), and the registration's resume mode (5th column: eager, lazy, or empty when it follows the install)
```
...
| `hooks.json` | Per-pane resume hooks (pane → event → registration). An event's value is either the command as a string or an object carrying the command alongside its settings; both shapes are permanently valid, neither deprecates the other, and there is no migration between them — the writer picks by whether there is anything to carry, so an entry that is only a command stays a plain string and a hand-edited file goes on looking as it does today. Portal creates an empty `hooks.json.lock` beside it on the first change, which it locks to keep concurrent writers from overwriting each other, and never removes. | `PORTAL_HOOKS_FILE` |
| `prefs.json` | UI preferences: last-used session-list grouping mode; the theme setting — `theme` for a pinned theme, or the `theme_light` / `theme_dark` pair; and `resume_mode`, holding `eager` or `lazy` — the install-wide default a resume hook that names no mode of its own inherits | `PORTAL_PREFS_FILE` |

...
// the Run / RunRaw shims wrap them via tmux.WrapCommandError for production
// parity. Without that wrap, GetServerOption's discriminator (which uses
// errors.As to recover *CommandError stderr) cannot tell absence from
// transport failure under integration tests driving real tmux.

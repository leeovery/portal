...
func AttachCommand(session string, exe ExecutableResolver, getenv func(string) string) ([]string, error) {
	...
	return composeAttachArgv(p, getenv("PATH"), session), nil
}
...
func composeAttachArgv(exePath, path, session string) []string {
	return []string{
		"/usr/bin/env", "-u", "TMUX", "-u", "TMUX_PANE",
		"PATH=" + path,
		exePath, "attach", session,
		// Phase 3 appends the ack token here: "--spawn-ack", batch+":"+token.
	}
}

...
// renderJoinedPanel is the shared single-tone, hand-drawn, border-joined modal
// panel — the chrome the help modal (§8.5) and the kill modal (§8.3) both compose
// through. It draws a rounded box (╭─╮ / │…│ / ╰─╯) whose EVERY glyph — corners,
// sides, and the compartment dividers — renders in border.separator (single-tone:
// the 2-tone footer leg of §8.1 was dropped in task 3-4), with the dividers joined
// to the side borders via real ├/┤ junctions.
//
...
func renderJoinedPanel(compartments [][]string, mode theme.Mode, colourless bool) string {
	contentWidth := 0
	totalRows := 0

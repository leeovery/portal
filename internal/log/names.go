...
// builders the rotation machinery will need — the size-cap `.N` segment names
// (Task 2-6) and the `portal.log.swept.<date>` retention sentinel (Task 2-8) —
// are deliberately NOT added yet: each would be unused-by-production until its
// owning task wires it, and the `unused` linter (staticcheck U1000) flags
// unexported functions with no callers. They are introduced alongside their
// first call site.

...
Identifies gap topics across completed research files and completed discussions, and adds them to the discovery map as fresh inception items with `source: gap-analysis` provenance. The orchestrator handles the cache check; this reference is invoked only when the cache is `stale`.
...
node .claude/skills/workflow-manifest/scripts/manifest.cjs set {work_unit}.inception.{name} routing {routing-from-C}
node .claude/skills/workflow-manifest/scripts/manifest.cjs set {work_unit}.inception.{name} source gap-analysis
```
...
Update the existing cache file at `.workflows/{work_unit}/.state/inception-gap-analysis.md` (pure markdown, no frontmatter):
...
Update the manifest's gap_analysis_cache (note: now lives under `phases.inception`, not `phases.discussion`):

```bash
node .claude/skills/workflow-manifest/scripts/manifest.cjs set {work_unit}.inception gap_analysis_cache.checksum "{computed-checksum}"
node .claude/skills/workflow-manifest/scripts/manifest.cjs set {work_unit}.inception gap_analysis_cache.generated "{ISO timestamp}"
node .claude/skills/workflow-manifest/scripts/manifest.cjs set {work_unit}.inception gap_analysis_cache.input_files '[]'
# Push one entry per input file (completed research + completed discussion):
node .claude/skills/workflow-manifest/scripts/manifest.cjs push {work_unit}.inception gap_analysis_cache.input_files "{file}.md"
```
